...
function createRoom(name) {
  ...
  rooms[id] = {
    id, name: name || `방 ${nextRoomId-1}`,
    players: {}, monsters: {}, bullets: [],
    state: 'waiting', stage: 1,
    kills: 0, totalKills: 0, tick: 0,
...
function generateMap(room) {
  ...
  room.rooms_data = [];
  const roomCount = 14;
  for (let i = 0; i < roomCount * 15 && room.rooms_data.length < roomCount; i++) {
...
function isWall(room, x, y) {
  ...
  return room.tiles[row*COLS+col]!==1;
}
...
function spawnMonster(room, type) {
  ...
  room.monsters[id] = {
    id, type, x:(r.x+1+Math.floor(Math.random()*(r.w-2)))*TILE+TILE/2,
    y:(r.y+1+Math.floor(Math.random()*(r.h-2)))*TILE+TILE/2,
    hp:cfg.hp, maxHp:cfg.hp, atk:cfg.atk, spd:cfg.spd,
    range:cfg.range, cd:0, cdMax:cfg.cd, size:cfg.size,
    score:cfg.score, label:cfg.label, alive:true, enraged:false,
  };
}
...
function initPlayer(room, pid) {
  const r = room.rooms_data[0];
  const ci = Object.keys(room.players).length % COLORS.length;
  return {
    id:pid, name:'용사',
    x:r.cx*TILE+(Math.random()-0.5)*40, y:r.cy*TILE+(Math.random()-0.5)*40,
    hp:120, maxHp:120, alive:true, iframes:0, shieldActive:0,
...
function gameTick(room) {
  ...
  for(const m of Object.values(room.monsters)){
    ...
    if(minD<m.range){
      ...
      if(m.range>150){
        ...
        target.hp-=m.atk; target.iframes=50;
  ...
  // 전멸 체크
  if(ps.length>0&&alive.length===0){
    broadcastRoom(room,{type:'game_over'});
    room.state='ended'; return;
  }
  ...
  if(room.bossSpawned&&!bossAlive){
    ...
    room.stage++; room.bossSpawned=false;
    initMonsters(room);
...
function broadcastRoomList(){
  // 방 목록이 변경될 때 모든 로비 클라이언트에 알림 (선택사항)
}
...
wss.on('connection', ws => {
  ...
  ws.on('message', data => {
    ...
    if(msg.type==='create_room'){
      const room=createRoom(msg.roomName||(window?.playerNickname||'새 방'));
      generateMap(room);
      initMonsters(room);
      room.state='playing';
      pid='p'+(room.nextId++);
      const player=initPlayer(room,pid);
      player.name=msg.name||'용사';
      player.ws=ws;
      room.players[pid]=player;
      playerRoom=room;
      ws.send(JSON.stringify({
        type:'init', pid, colorIdx:player.colorIdx, name:room.name,
    ...
    if(msg.type==='join_room'){
      ...
      broadcastRoom(room,{type:'player_join',name:player.name,count:Object.keys(room.players).length},ws);
      return;
    }
    ...
    if(msg.type==='name'){ player.name=(msg.name||'용사').slice(0,12); return; }
    ...
    if(msg.type==='attack'&&player.alive){
      ...
      broadcastRoom(playerRoom,{type:'attack_fx',x:player.x,y:player.y,pid});
    }
    if(msg.type==='bomb'&&player.alive){
      for(let i=0;i<8;i++) playerRoom.bullets.push({x:player.x,y:player.y,angle:i*Math.PI/4,speed:6,dmg:50,life:40,isMob:false,pid});
    }
    if(msg.type==='shield'&&player.alive){ player.shieldActive=180; }
    if(msg.type==='thunder'&&player.alive){
      ...
      broadcastRoom(playerRoom,{type:'thunder_fx',x:player.x,y:player.y,r:160});
    }
  });
  ...
  ws.on('close', ()=>{
    ...
    if(remaining===0){ delete rooms[playerRoom.id]; }
  });
});
